...
window.onload = function() 
{
	...
	var level = {
		...
		tiles: [],      // The two-d     cccccccccccccccc cccccccccccccccccimensional tile array
		selectedtile: { selected: false, column: 0, row: 0 }
	};
	
	// All of the different tile colors in RGB
	  var sounds=[["1.mp3"],["1.mp3"],["1.mp3"],["1.mp3"],["1.mp3"],["1.mp3"],["1.mp3"],["1.mp3"]];
	  var producttype=[["1.jpg"],["2.png"],["1313.png"],["4.png"],["5.png"],["6.png"],["7.png"],["8.png"]];
	  var bigproduct=[["11.png"],["22.png"],["1313.png"],["44.png"],["55.png"],["66.png"],["77.png"],["88.png"]];

	  var tilecolors=[["1.jpg"],["2.jpg"],["13.png"],["4.jpg"],["5.jpg"],["6.jpg"],["7.jpg"],["8.jpg"]];
	...
	var buttons = [ { x: 30, y: 240, width: 150, height: 50, text: "New Game"},
					...
					{ x: 30, y: 360, width: 150, height: 50, text: "Enable Auto"}];
	...
	function render() {
		...
		context.fillRect(level.x, level.y,levelwidth+20, levelheight +15);
		...
		if (gameover) {
		    ...
		    imgo.src="body.jpg";  
		// context.drawImage(imgo,levelwidth+400, levelheight+200);
			context.fillStyle = "rgba(0, 0, 0, 0.8)";
			context.fillRect(level.x-65, level.y, levelwidth+400, levelheight+19);

			
			context.fillStyle = "#ffffff";
			context.font = "24px Verdana";
			drawCenterText("Game Over!Happy UAE national day", level.x, level.y + levelheight / 2 + 10, levelwidth);
		}
	}
	...
	function drawFrame() {
		...
		var imgo=document.createElement('img');
		 imgo.src="background.png";  
		context.drawImage(imgo,0,0);
		// context.fillStyle = "orange";
		// context.fillRect(0,0,canvas.width,canvas.height);

		// context.fillStyle = "transparent";
		// context.fillRect(1,1,canvas.width,canvas.height-34);
		
		// // Draw header
		// context.fillStyle = "transparent";
		// context.fillRect(180, 0, canvas.width, 124);

		// context.fillStyle = "transparent";
		// context.fillRect(180,443, canvas.width, 124);
		
		// // Draw title
		// context.fillStyle = "black";
		// context.font = "24px Verdana";
		// context.fillText("UNIKAI PRODUCT MATCH", 10, 30);
	...
	var buttonColors=["1.png","2.png","3.png"];
	// Draw buttons
	...
	function renderTiles() {
		for (var i=0; i<level.columns; i++) {
			for (var j=0; j<level.rows; j++) {
				...
				if (level.tiles[i][j].type >= 0) {
					...
					var col =producttype[level.tiles[i][j].type];

					// Draw the tile using the color
					drawTile(coord.tilex, coord.tiley, col, col[1], col[2]);
				}
		...
		if (gamestate == gamestates.resolve && (animationstate == 2 || animationstate == 3)) {
			...
			drawTile(coord1.tilex, coord1.tiley, 45, 230, 90);
			drawTile(coord2.tilex, coord2.tiley, 23, 77, 80);
			
			// Change the order, depending on the animation state
			if (animationstate == 2) {
				// Draw the tiles
				drawTile(coord1shift.tilex, coord1shift.tiley, col1, col1[1], col1[2]);
				drawTile(coord2shift.tilex, coord2shift.tiley, col1, col2[1], col2[2]);
			} else {
				// Draw the tiles
				drawTile(coord2shift.tilex, coord2shift.tiley, col2, col2[1], col2[2]);
				drawTile(coord1shift.tilex, coord1shift.tiley, col1, col1[1], col1[2]);
			}
		}
	}
	...
	function drawTile(x, y, r, g, b) {
        ...
        context.strokeRect(x+10,y+10,50,50);
			...
			for (var i=0; i<level.columns; i++) {
				...
				if (i == level.columns-1) {
					...
					if (level.tiles[i][j].type == level.tiles[i+1][j].type &&
						...
						// cons.innerHTML+=(level.tiles[i][j].type)+":<br/>";
	...
	function removeClusters() {
		...
		loopClusters(
			...
			p.innerHTML="<img  height=500 width=400 src='"+bigproduct[level.tiles[column][row].type]+"'/>";	
			p.innerHTML+="<audio  autoplay><source src='"+sounds[level.tiles[column][row].type]+"'type='audio/mpeg'>";
		...
		for (var i=0; i<level.columns; i++) {
			for (var j=level.rows-1; j>=0; j--) {
				...
				if (level.tiles[i][j].type == -1) {
					...
					var shift = level.tiles[i][j].shift;
					if (shift > 0) {
					// cons.innerHTML="<img src='"+tilecolors[level.tiles[i][j+shift].type]+"'>"
	...
	function onMouseMove(e) {
		...
		if (drag && level.selectedtile.selected) {
			...
			if (mt.valid) {
				// Valid tile whether it is draged or not
		...
		for (var i=0; i<buttons.length; i++) {
			if (pos.x >= buttons[i].x && pos.x < buttons[i].x+buttons[i].width &&
				...
				if (i == 0) {
					...
					// AI Bot
					aibot = !aibot;
					buttons[i].text = (aibot?"Disable":"Enable") + "Auto ";
				}
			}
		}
	}
